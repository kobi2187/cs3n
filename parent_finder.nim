...
proc determineParentId(obj: Construct; data: AllNeededData): (bool, Option[UUID]) =
  ...
  var ignoredConstructs = @["IdentifierName", "QualifiedName", "BlockStarts", "AliasQualifiedName","GlobalStatement","IncompleteMember"]
  var ckIgnoredConstructs = @[ ckAliasQualifiedName,ckGlobalStatement,ckIncompleteMember]
  # Some issue with roslyn 3.7. sometimes mistakes, and defaults to GlobalStatement as a parent.
  ...
  #   let last = state.getLastBlocks(ckMethod, obj.id.get) # FIXME: add more according to cases.
  #   echo last
  ...
  of ckContinueStatement:
    let lastMatch = getLastBlocks(@["ForStatement", "ForEachStatement", "WhileStatement", "IfStatement", "SwitchSection", "CatchClause", "DoStatement"], obj.id.get)
    assert lastMatch.isSome
    res = lastMatch.get.id.some
  ...
  of ckBreakStatement: # if, case, else, while, do, ...others?
    let lastMatch = getLastBlocks(@[ ckCaseSwitchLabel, ckDefaultSwitchLabel, ckIfStatement, ckTryStatement, ckElseClause, ckSwitchSection, ckWhileStatement, ckMethod],obj.id.get)
    assert lastMatch.isSome
    res = lastMatch.get.id.some
...
proc getParent*(root: var CsRoot; newobj: Construct; allData: AllNeededData): (
  ...
  if pid.isNone: assert dis == true,"both no parent id, and not discarded"
